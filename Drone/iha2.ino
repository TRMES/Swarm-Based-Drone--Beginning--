//---------- PINS ----------
#define FRONT_LEFT_MOTOR_PIN 6
#define FRONT_RIGHT_MOTOR_PIN 5
#define REAR_LEFT_MOTOR_PIN 4
#define REAR_RIGHT_MOTOR_PIN 3
#define INTERRUPT_PIN 2

//---------- NRF24 PINS ----------
#define NRF24_CE_PIN 9
#define NRF24_CSN_PIN 10

//---------- IMU CALIBRATION ----------
#define ACCEL_OFFSET_X 155.61 * MPU9250::CALIB_ACCEL_SENSITIVITY / 1000.f
#define ACCEL_OFFSET_Y -42.25 * MPU9250::CALIB_ACCEL_SENSITIVITY / 1000.f
#define ACCEL_OFFSET_Z -27.71 * MPU9250::CALIB_ACCEL_SENSITIVITY / 1000.f
#define GYRO_OFFSET_X -0.78 * MPU9250::CALIB_GYRO_SENSITIVITY
#define GYRO_OFFSET_Y 0.90 * MPU9250::CALIB_GYRO_SENSITIVITY
#define GYRO_OFFSET_Z -0.36 * MPU9250::CALIB_GYRO_SENSITIVITY

//---------- RECEIVER & TRANSMITTER ---------
#define TRANSMITTER_JOYSTICK_MIN_VALUE 0
#define TRANSMITTER_JOYSTICK_MAX_VALUE 1023
#define TRANSMITTER_JOYSTICK_DEAD_BAND 30
#define TRANSMITTER_ARMING_DURATION_IN_MILLISECONDS 1000
#define TRANSMITTER_ARMING_JOYSTICK_TOLERANCE 20

//---------- ESC's ---------
#define MIN_MOTOR_PULSE_WIDTH 1000
#define MAX_MOTOR_PULSE_WIDTH 2000

//---------- TIMEOUTS ---------
#define IMU_COMMUNICATION_TIMEOUT_IN_MILLISECONDS 100
#define RECEIVER_COMMUNICATION_TIMEOUT_IN_MILLISECONDS 500
#define PROGRAM_TIMEOUT_IN_MILLISECONDS WDTO_1S// Watch Dog Timer Parameter

//---------- LIMITS ---------
#define THROTTLE_START_POINT 10 // between 0-180
#define THROTTLE_LIMIT_POINT 70 // between 0-180
double QUADCOPTER_MAX_TILT_ANGLE = 20.00; // roll, pitch tilt angle limit in degrees
double QUADCOPTER_MAX_YAW_ANGLE_CHANGE_PER_SECOND = 90.00;

//---------- PID CONFIGURATION ---------
double KP_roll_pitch = 0.80;
double KI_roll_pitch = 0.01;
double KD_roll_pitch = 0.20;

double KP_yaw = 0.0;
double KI_yaw = 0.0;
double KD_yaw = 0.0;

//---------- PID CONTROL LIMITS ---------
double ROLL_PITCH_CONTROL_SIGNAL_LIMIT = KP_roll_pitch * QUADCOPTER_MAX_TILT_ANGLE * 2;
